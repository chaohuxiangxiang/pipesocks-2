      ...
      case Initiated:
        {
          ...
          if (qvm["protocol"] == "TCP")
          ...
          else if (qvm["protocol"] == "UDP")
            {
              ...
              connect(usock,SIGNAL(RecvData(QHostAddress,unsigned short,QByteArray)),
                      this,SLOT(UDPRecv(QHostAddress,unsigned short,QByteArray)));
              status = UDP;
...
void Pump::EndSession()
{
    ...
    if (csock->state() == QAbstractSocket::UnconnectedState
        && (ssock == NULL || ssock->state() == QAbstractSocket::UnconnectedState))
      {
        if (usock)
            usock->close();
        deleteLater();
